...
//The given states is state keyed by the route of the handler it came from.
export function renderRoutes(routes, states) {
  ...
  .then(() => {
    ...
    //TODO: important to remember to routes.path API
...
export function renderPage(renderObject, zygo) {
  ...
  return runSerialize(renderObject.routes, renderObject.states)
    .then(() => {
      let templateData = {
      ...
      //TODO: getting the context object across and serialized
      //TODO: what about renderObject.routes? where does this fit in.
      //Will need a new client side API.
  ...
  return Promise.all(routes.map(getHandler))
    .then(() => {
      handlers.map((handler, i) => {
        if (handler && handler.serialize) handler.serialize(states.context, states[routes[i].path]);
        //TODO integrate the states.context API
      });
    });
