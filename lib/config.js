...
export function desugarRoutes(route, parent=null) {
  ...
  return Promise.all(
    Object.keys(route).map((key) =>
      desugarKeyValue(key, route[key], parent)
...
function desugarKeyValue(key, value, parent) {
  if (key === "mixin") return handleMixins(key, value);
  if (key === "component") return handleString(key, value);

  if (key !== "default" && key[0] !== "/") throw new Error("Invalid route key: " + key);
  ...
  function handleString(key, value) {
    ...
    if (key === "default") return handleModule(value);
