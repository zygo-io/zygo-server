...
export default class Config {
  ...
  parse() {
    ...
    return this._getFile(this.configPath).then((config) => {
      ...
      this.config.template = files[0];
      this.config.routes = JSON.parse(files[1]);
      this.config.clientRoutes = JSON.parse(files[2]);
      this.config.serverRoutes = JSON.parse(files[3]);
    });
  }
