...
export function parse(configPath, spec=zygoParseSpec) {
  ...
  return resolvePath(configPath, process.cwd())
    ...
    .then(JSON.parse)
    .then((json) => {
      //Initially set result to the json, then custom parse the spec.
      //This allows the user to set custom config options etcetera.
      result = json;
      return Promise.all(
        Object.keys(spec)
...
//Desugar routes.json. Syntactic convenience, doesn't belong in actual code.
//We allow specifying subroutes as components directly for convenience.
export function desugarRoutes(route) {
