...
export function build(zygo) {
  return getPageObjects(zygo.routes)
    .then(optimize)
...
function _build(bundles, zygo) {
  ...
  return Promise.all(
    bundles.map((bundle) => {
      ...
      //Build the bundle!
      return builder.buildTree(bundle.tree, filePath)
        .then(() => {
  ...
  return traverse(routes, '', (path, route) => {
    return Promise.resolve()
      ...
      .then((module) => {
        ...
        if (module && typeof module.default.clientHandler === "string") pages[path].push(module.default.clientHandler);
        else if (module && typeof module.default.handler === "string") pages[path].push(module.default.handler);

        //We first normalize the paths relative to the component, as we allow relative requires for handlers.
        //We trace dependencies, and filter out css (as systemJS doesn't support bundling this atm)
        return Promise.all(
          pages[path].map((modulePath) =>
            jspm.normalize(modulePath, route.component)
              .then((modulePath) => builder.trace(modulePath))
              .then((trace) => {
                var result = {
...
function traverse(route, path, callback) {
  return callback(path, route)
    .then(() =>
      Promise.all(
        Object.keys(route)
          .filter((key) => key[0] === '/' || key === "default")
          .map((key) =>
            Promise.all(
              route[key].map((subRoute) =>
                traverse(subRoute, path+key, callback))
            )
          )
      )
    )
