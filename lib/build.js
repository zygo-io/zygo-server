...
function _build(bundles, zygo) {
  ...
  bundles.map((bundle) => {
    ...
    .update(Object.keys(bundle.tree).toString())
    .digest('hex');
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.writeFile(bundlesPath, JSON.stringify(bundlesJSON), (error) => {
      ...
      //TODO: we really only want to save the bundlesJSON path
...
function getPageObjects(routes) {
  ...
  return traverse(routes, '', (path, route) => {
    ...
    if (route.clientHandler) pages[path].push(route.clientHandler);
    else if (route.handler) pages[path].push(route.handler);

    return Promise.all( pages[path].map((module) => builder.trace(module)) )
