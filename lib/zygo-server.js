...
export default class Zygo extends EventEmitter {
  ...
  _getRouteObject(path, headers={}, requestMethod="GET") {
    ...
    function _handleMatch(routeString) {
      ...
      return runHandler(loadingRoute)
        .then((meta) => {
...
//Runs the handler for the route, returns a promise resolving
// to the metadata returned by handler function if it exists, else
// an empty object.
//We allow for handlers to actually be objects with a component
// and meta property, so we check for that.
