...
//             handle situations where routes/clientRoutes/serverRoutes is not defined
...
export default class Zygo extends EventEmitter {
  ...
  _getRouteObject(path, headers={}, requestMethod="GET") {
    for (let routeString in this.routes) {
      ...
      if (match) {
        let handlers = this.routes[routeString];
        ...
        return this._runHandlers(loadingRoute).then((result) => {
    ...
    return loadingRoute.handlers.reduce((handlerChain, nextHandler) => {
      return handlerChain
        ...
        return new Promise((resolve, reject) => {
          ...
          return resolve(jspm.import(nextHandler).then((handlerModule) => {
            return handlerModule.default(this.state, loadingRoute);
          }));
        });
      });
    }, Promise.resolve());
